...
<main><section class="container page-shell">
  ...
  <c:if test="${not empty hijos}">
    ...
    <section class="parent-summary-card" data-specialty="${selectedEspecialidad}"><div class="parent-summary-card__heading"><div><div class="section-heading">Materias de <c:forEach var="hijo" items="${hijos}"><c:if test="${hijo.id == selectedAlumnoId}"><c:out value="${hijo.nombre}" /></c:if></c:forEach></div></div></div>
      <c:choose><c:when test="${not empty selectedSummary}"><c:forEach var="item" items="${selectedSummary}"><details class="parent-subject-accordion" ${item.planillaId == selectedPlanillaId ? 'open' : ''}><summary><span class="parent-subject-accordion__title"><strong><c:out value="${item.materiaNombre}" /></strong><small><c:out value="${item.tareasCount}" /> tareas registradas</small></span><span class="parent-subject-accordion__grade"><strong><c:out value="${item.puntos}" /> / <c:out value="${item.totalPosible}" /> pts</strong><small>Nota <c:out value="${item.nota}" /> · <c:out value="${item.porcentaje}" />%</small></span></summary><c:choose><c:when test="${not empty tareasPorPlanilla[item.planillaId]}"><div class="parent-task-table-wrap"><table class="grade-table parent-task-table"><thead><tr><th>Tarea</th><th>Fecha</th><th>Puntos</th></tr></thead><tbody><c:forEach var="grade" items="${tareasPorPlanilla[item.planillaId]}"><tr><td><c:out value="${grade.tareaTitulo}" /></td><td><c:out value="${grade.fecha}" /></td><td><strong><c:out value="${grade.puntos}" /> / <c:out value="${grade.total}" /></strong></td></tr></c:forEach></tbody></table></div></c:when><c:otherwise><div class="parent-task-empty">Esta materia todavía no tiene tareas cargadas.</div></c:otherwise></c:choose></details></c:forEach></c:when><c:otherwise><div class="empty-state">No hay materias con tareas cargadas para este alumno.</div></c:otherwise></c:choose>
    </section>
  </c:if>
