...
export function EmptyState({ title, description, icon = <Icon name="empty" />, action }: EmptyStateProps) {
  return <div className="ds-empty"><span className="ds-empty-icon" aria-hidden="true">{icon}</span><h3 className="ds-h3">{title}</h3>{description && <p className="ds-secondary">{description}</p>}{action}</div>;
}
