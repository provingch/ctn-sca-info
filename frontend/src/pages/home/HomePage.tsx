...
export default function HomePage() {
  ...
  return <AppShell title="Panel SCA del curso"><div className="toolbar filters"><button className="button secondary" onClick={() => setSearch({})}>← Inicio</button><select value={data.selCurso?.id || ''} onChange={(e) => params({ cursoId: e.target.value })}><option value="">Seleccioná un curso</option>{data.cursos.map((c) => <option key={c.id} value={c.id}>{c.curso}° {c.seccion} · {c.especialidad}</option>)}</select><select value={data.selEtapa} onChange={(e) => params({ etapa: e.target.value })}><option value="1">Primera etapa</option><option value="2">Segunda etapa</option></select><button className={`tab ${view === 'clase' ? 'active' : ''}`} onClick={() => params({ view: 'clase' })}>Clase</button><button className={`tab ${view === 'planillas' ? 'active' : ''}`} onClick={() => params({ view: 'planillas' })}>Planillas</button></div>
