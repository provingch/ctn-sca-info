...
export default function PlanillaPage() {
  ...
  const [reabriendoEtapa, setReabriendoEtapa] = useState<1 | 2 | null>(null);
  ...
  async function confirmarEtapa1Accion() {
    ...
    try {
      ...
      setStatus(e instanceof ApiError ? e.message : 'No se pudo confirmar Etapa 1.');
    } finally {
  ...
  async function reformatearEtapa1Accion() {
    if (!data) return;
    setReformattingEtapa1(true);
    try {
      await reformatearEtapa1(id);
      setStatus('Etapa 1 reformateada.');
      applyPlanillaData(await getPlanilla(id));
    } catch (e) {
      setStatus(e instanceof ApiError ? e.message : 'No se pudo reformatear Etapa 1.');
    } finally {
      setReformattingEtapa1(false);
    }
  }

  // Acción de admin con impacto real: vuelve a permitir editar una etapa ya cerrada, por eso pide un motivo.
  // Si falla, el error lo muestra el diálogo (que conserva el motivo) y no se cierra.
  async function confirmarReapertura(etapa: 1 | 2, motivo: string) {
    await reabrirEtapa(id, etapa, motivo);
    setReabriendoEtapa(null);
        ...
        <div className="planilla-legacy-toolbar">
          ...
          {(data.planilla.etapaIndex === 1 || data.planilla.etapaIndex === 2) && (
            <div className="planilla-toolbar-actions">
              ...
              {isGlobalAdmin && isEtapa1Locked && <button className="button secondary" type="button" onClick={() => setReabriendoEtapa(1)}>Reabrir Etapa 1</button>}
              {isGlobalAdmin && isEtapa2Locked && <button className="button secondary" type="button" onClick={() => setReabriendoEtapa(2)}>Reabrir Etapa 2</button>}
              {reabriendoEtapa && <ReabrirEtapaDialog etapa={reabriendoEtapa} onCancel={() => setReabriendoEtapa(null)} onConfirm={(motivo) => confirmarReapertura(reabriendoEtapa, motivo)} />}
