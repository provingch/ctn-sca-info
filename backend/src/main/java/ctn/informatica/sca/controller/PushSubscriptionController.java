...
public class PushSubscriptionController {
    ...
    public PushSubscriptionResponse getPushSubscription(Authentication authentication) {
        ...
        try {
            boolean subscribed = !new PushSubscriptionDao().findByUser(userId, userType).isEmpty();
    ...
    public void savePushSubscription(
        ...
        try {
            boolean saved = new PushSubscriptionDao().save(userId, userType, request.endpoint(), request.p256dh(), request.auth());
            if (!saved) {
    ...
    public void unsubscribePushSubscription(Authentication authentication) {
        ...
        String userType = resolveUserType(userId, authentication);
        try {
            new PushSubscriptionDao().deleteByUser(userId, userType);
        } catch (SQLException ex) {
    ...
    public void testPushSubscription(Authentication authentication) {
        ...
        try {
            ...
            int subscriptionCount = new PushSubscriptionDao().findByUser(userId, userType).size();
